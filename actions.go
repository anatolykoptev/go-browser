...
type Action struct {
	Type          string        `json:"type" jsonschema:"Action type: click, type_text, wait_for (CSS selector, text, text_gone, or wait_ms for time-based wait), snapshot (accessibility tree — best for AI), screenshot (returns snapshot text by default; use format='image' for JPEG, format='full' for full-page JPEG), evaluate (any JS expression), eval_on_new_document, press (supports F1-F12), sleep/wait, navigate, set_cookies, handle_dialog, get_cookies, destroy_session, hover, go_back, get_logs, warmup, scroll, select_all (select all text via JS — use instead of Ctrl+A for TipTap/ProseMirror), insert_text_input_event (fill TipTap/ProseMirror contenteditable via synthetic beforeinput+input events with inputType=insertReplacementText — use when type_text and select_all don't take effect; requires selector + text; returns a diagnostic string 'dispatched:cancelled=...|now=...'), select_option (select dropdown values by text), get_storage, set_storage, clear_storage. Selectors support CSS, text=, xpath=, ref= prefixes. Use ref=eN after a snapshot action to click/type elements by their ref id. screenshot without format returns snapshot text (saves tokens)."`
	Selector      string        `json:"selector,omitempty" jsonschema:"Element selector for click/type_text/wait_for/hover/scroll/select_all. Supports: CSS (#id, .class), text=Text, xpath=//div, ref=eN (from snapshot). Best practice: snapshot first, then use ref=eN for precise targeting. For select_all: if empty, selects all in focused element; if specified, selects all in that element."`
