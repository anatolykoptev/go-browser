...
func TestExecuteAction_UnknownType(t *testing.T) {
	result := ExecuteAction(context.Background(), nil, Action{Type: "nonexistent_action"}, nil, nil, false, nil, 0)
	if result.Ok {
...
func TestExecuteAction_DestroySessionNoOp(t *testing.T) {
	result := ExecuteAction(context.Background(), nil, Action{Type: "destroy_session"}, nil, nil, false, nil, 0)
	if !result.Ok {
...
func TestExecGetLogs_DefaultLimit(t *testing.T) {
	...
	result := ExecuteAction(context.Background(), nil, Action{Type: "get_logs"}, nil, logs, false, nil, 0)
	if !result.Ok {
